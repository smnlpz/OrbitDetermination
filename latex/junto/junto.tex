...
Los tres valores $x$, $y$, $z$, a los que en conjunto llamaremos $x(t)$, forman una solución del problema de Kepler. Además, sabemos la fórmula de las soluciones para el problema de Kepler son:
\[
x(t)=a(\cos{u(t)}-e,\sqrt{1-e^2}\sin{u(t)})
\]

\noindent donde $u(t)$ es la anomalía. Dado que la ecuación está formada por funciones trigonométricas, que son analíticas, y la anomalía es una función analítica por el teorema de la función implícita, llegamos a que $x(t)$ será también una función analítica.\\
...
\left\{
\begin{array}{l}
	\lambda=c_0+c_1t+c_2t^2+...\\
	\lambda_1=c_0+c_1t_1+c_2t_1^2+...\\
	\lambda_2=c_0+c_1t_2+c_2t_2^2+...\\
	\lambda_3=c_0+c_1t_3+c_2t_3^2+...
\end{array}
\right.
...
\begin{array}{cccc}
\lambda   & 1 & t & t^2\\
\lambda_1 & 1 & t_1 & t^2_1\\
\lambda_2 & 1 & t_2 & t^2_2\\
\lambda_3 & 1 & t_3 & t^2_3\\
\end{array}
\right|
=0
\label{eq:resultante}
...
\begin{align}
\lambda_L=
\frac{(t-t_2)(t-t_3)}{(t_1-t_2)(t_1-t_3)}\lambda_1
+\frac{(t-t_3)(t-t_1)}{(t_2-t_3)(t_2-t_1)}\lambda_2
+\frac{(t-t_1)(t-t_2)}{(t_3-t_1)(t_3-t_2)}\lambda_3
...
\noindent donde $\min{(t,t_1,t_2,t_3)}<\xi<\max{(t,t_1,t_2,t_3)}$. El cálculo de $\xi$ es complicado, pero podremos tomar una cota superior para $\lambda^{\romannumeral 4}$. Dado que $(\lambda,\mu,\nu)$ es un vector unitario, cada una de las coordenadas ha de ser menor que 1; además, la derivada de un vector unitario es unitaria\cite{unitary}, de manera que las coordenadas de la derivada n-ésima son menores que 1, y $(\lambda,\mu,\nu)^{(N)}=(\lambda^{(N)},\mu^{(N)},\nu^{(N)})$. Por tanto:
\begin{align}
|\lambda(t)-\lambda_L(t)|\leq\ddfrac{1}{4!}(t-t_1)(t-t_2)(t-t_3)
...
\begin{align}
|\lambda(t)-\lambda_L(t)|\leq\ddfrac{1}{(N+1)!}(t-t_1)...(t-t_N)
...
Con el estudio del error y todo lo visto anteriormente, podemos justificar que los valores de $t$ han de ser pequeños en la práctica para que las cantidades aproximadas calculadas sean lo más cercanas a su valor original. Además, no tendría sentido una diferencia de tiempo muy grande entre las observaciones del objeto, pues pasaríamos mucho tiempo para determinar una única órbita.\\
...
\begin{align*}
\lambda'_L = \frac{2t-(t_2+t_3)}{(t_1-t_2)(t_1-t_3)}\lambda_1
+\frac{2t-(t_3+t_1)}{(t_2-t_3)(t_2-t_1)}\lambda_2
+\frac{2t-(t_1+t_2)}{(t_3-t_1)(t_3-t_2)}\lambda_3\\
\lambda''_L = \frac{2}{(t_1-t_2)(t_1-t_3)}\lambda_1
+\frac{2}{(t_2-t_3)(t_2-t_1)}\lambda_2
+\frac{2}{(t_3-t_1)(t_3-t_2)}\lambda_3
\end{align*}

Tal y como comentamos anteriormente, se procederá al cálculo de $\mu$ y $\nu$ y al estudio de su error de aproximación de manera similar a la desarrollada anteriormente.
...
A partir de la figura superior, del producto escalar de $(X,Y,Z)$ con $(\lambda,\mu,\nu)$ y del teorema de los senos ($\ddfrac{a}{\sin{\alpha}}=\ddfrac{b}{\sin{\beta}}=\ddfrac{c}{\sin{\gamma}}$), podemos escribir las siguientes ecuaciones:
\begin{align}
\left\{
\begin{array}{l}
...
Si sustituimos dichas ecuaciones en la ecuación de $\rho$ en \eqref{eq:rho_values}, esta pasará a ser:
\[
...
El valor para $M$ y $m$ será conocidos cuando $M$ sea positivo. Busquemos ahora la solución de esta ecuación para $\phi$; dado que $\rho=0$ y $r=R$ es una solución al problema, podremos asegurar que $\phi=\pi-\psi$ es una solución válida, aunque no es de valor práctico ya que que ésta corresponde a la posición del observador en la Tierra. Por tanto, dado que la solución ha de pertenecer al problema físico y que nos encontramos en un triángulo, podemos asegurar que:
\[
\phi<\pi-\psi
\]
...
%\label{subsec:newton}
%Tras haber realizado un estudio teórico, hagamos ahora un estudio numérico de la ecuación \eqref{eq:phi_solution}. Dado que no podemos obtener los valores exactos de $\phi$ con los que se cumple dicha ecuación, utilizaremos el método de Newton para obtener un valor aproximado de éstos. Comencemos recordando dicho método.\\

%Consideramos la ecuación $f(x)=0$ y $x^*$ tal que $f(x^*)=0$. Dado $x_0$, notamos por $\{x_n\}_{n\in\mathbb{N}}$ la sucesión de iterantes del método de Newton-Raphson,
%\[
%x_{n+1}=x_n-\ddfrac{f(x_n)}{f'(x_n)}, n\geq0
%\]

%\begin{proposition}
%Sea $f\in\mathcal{C}([x^*-\alpha,x^*+\alpha])$, con $\alpha>0$ y $f'(x^*)\neq0$; entonces existe $\varepsilon>0$ tal que para todo $x_0$ con $|x_0-x^*|<\varepsilon$ se tiene:
%\begin{itemize}
%\item la sucesión $\{x_n\}_{n\in\mathbb{N}}$ está bien definida y converge a $x^*$.
%\item Además, cuando $f''(x^*)\neq0$, dicha sucesión tiene orden de convergencia 2.\\
%\end{itemize}
%\end{proposition}

%Para nuestro caso definiremos $f(\phi)=\sin^4{\phi}-M\sin{(\phi+m)}$, cuya derivada será $f'(\phi)=4\sin^3{\phi}\cos{\phi}-M\cos{(\phi+m)}$. A continuación, escribimos la sucesión de iterantes para el método de Newton-Raphson:
%\begin{align}
%\phi_{n+1}=\phi_n-\ddfrac{\sin^4{\phi_n}-M\sin{(\phi_n+m)}}{4\sin^3{\phi_n}\cos{\phi_n}-M\cos{(\phi_n+m)}}, n\geq0
%\label{eq:phi_newton}
%\end{align}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Dado que $N\neq0$, el límite de las dos desigualdades será:
\[
\left[1+\ddfrac{3D_1}{DR^4}\cos{\psi}\right]=0
\]

Ahora, utilicemos \eqref{eq:rho_values} y la relación \eqref{eq:triangle_relations_1} vista anteriormente para eliminar $\cos{\psi}$ y $\frac{D_1}{D}$ de la expresión superior.
\[
\left\{
\def\arraystretch{2}
\begin{array}{l}
...
Si tomamos el miembro de la derecha de esta igualdad como una ecuación en $r$, tendremos que ésta alcanzará el mínimo en $r=0$, por lo que para cada valor de $r$ habrá un único valor positivo de $\rho$. Además, dado que estamos trabajando con el límite de las desigualdades \eqref{eq:condicion_unicidad}, todos los pares de valores $(\rho,r)$ que satisfagan la igualdad \eqref{eq:rho_cuadrado} se encontrarán en el límite de las regiones donde la solución es única, las cuales son superficies de revolución alrededor de la línea imaginaria que une la Tierra y el Sol. Para obtener una imagen más visual, representamos a continuación la sección de estas superficies con un plano que pase por la recta $SE$.
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{images/fig_35.png}
\caption{Sección de la superficie de revolución generada por los límites de \eqref{eq:condicion_unicidad}.}
\label{fig:fig_35}
\end{figure}
